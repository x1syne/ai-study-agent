...
export const SYSTEM_PROMPTS = {
  graphGeneration: `Ты — эксперт-методист. Создаёшь структуры курсов.
Раздели тему на модули и темы так, как считаешь нужным для полного освоения материала.
...
export function getGraphGenerationPrompt(goal: string, _level: string) {
  return `Создай структуру курса по теме: "${goal}"

Ты — эксперт-методист. Твоя задача — разбить тему на модули и темы так, чтобы студент мог полностью её освоить.

═══════════════════════════════════════════════════════════════
📋 ЧТО НУЖНО СДЕЛАТЬ:
═══════════════════════════════════════════════════════════════

1. Раздели тему "${goal}" на МОДУЛИ (логические разделы)
2. Каждый модуль раздели на ТЕМЫ (конкретные уроки)
3. Определи порядок изучения и зависимости между темами
4. Оцени сложность и время каждой темы
...
МОДУЛИ:
- Логические разделы курса
- Порядок от базового к продвинутому
- Каждый модуль имеет название, описание и иконку

ТЕМЫ:
- Конкретные уроки внутри модуля
- Одна тема = один навык или концепция
- Сложность: EASY, MEDIUM, HARD или EXPERT
- Время: от 10 до 45 минут
- prerequisites: список slug тем, которые нужно пройти перед этой

DOMAIN (предметная область):
Выбери одно: PHYSICS, MATHEMATICS, PROGRAMMING, CHEMISTRY, BIOLOGY, HISTORY, LANGUAGES, ECONOMICS, ARTS, MEDICINE, LAW, ENGINEERING, GENERAL
  ...
  "modules": [
    {
      ...
      "description": "Что изучим в этом модуле",
      "icon": "📚",
      "order": 1,
      "topics": [
        {
