...
1. ОПРЕДЕЛИ ПРЕДМЕТНУЮ ОБЛАСТЬ (domain):
   Проанализируй тему "${goal}" и определи к какой области она относится.
   Возможные значения domain:
   - PHYSICS — физика, механика, термодинамика, оптика, электричество
   - MATHEMATICS — алгебра, геометрия, анализ, статистика
   - PROGRAMMING — программирование, алгоритмы, веб-разработка, базы данных
   - CHEMISTRY — органическая, неорганическая химия, биохимия
   - BIOLOGY — анатомия, генетика, экология, микробиология
   ...
   - GENERAL — если тема не подходит ни к одной области

2. ПРОАНАЛИЗИРУЙ тему "${goal}":
   - Что это за область знаний?
   - Какие МОДУЛИ (разделы) логически входят в эту тему?
   - Какие ТЕМЫ входят в каждый модуль?

3. ПОСТРОЙ ИЕРАРХИЧЕСКУЮ СТРУКТУРУ:
   - Курс делится на МОДУЛИ (логические разделы)
   - Каждый модуль содержит ТЕМЫ (конкретные уроки)
   - Модули идут от простого к сложному
   - Темы внутри модуля связаны тематически

4. ФОКУСИРУЙСЯ НА ТЕМЕ:
...
Модуль 1: Основы ООП
  ...
  - Классы и объекты
  - Атрибуты и методы
Модуль 2: Конструкторы и инициализация
  - Метод __init__
...
Модуль 1: Основные понятия
  ...
  - Система отсчёта
  - Материальная точка
Модуль 2: Характеристики движения
  - Путь и перемещение
  - Скорость
  - Ускорение
Модуль 3: Виды движения
...
{
  "domain": "PHYSICS | MATHEMATICS | PROGRAMMING | CHEMISTRY | BIOLOGY | HISTORY | LANGUAGES | ECONOMICS | ARTS | MEDICINE | LAW | ENGINEERING | GENERAL",
  "modules": [
    {
      "name": "Название модуля",
      "description": "Описание что изучим в этом модуле (1-2 предложения)",
      "icon": "📚",
      "order": 1,
      "topics": [
        {
...
DOMAIN (ОБЯЗАТЕЛЬНО):
- Определи предметную область курса
- Используй ТОЛЬКО одно из 13 значений: PHYSICS, MATHEMATICS, PROGRAMMING, CHEMISTRY, BIOLOGY, HISTORY, LANGUAGES, ECONOMICS, ARTS, MEDICINE, LAW, ENGINEERING, GENERAL
- Если тема не подходит ни к одной области — используй GENERAL

МОДУЛИ:
- 3-6 модулей на курс
- Каждый модуль имеет уникальное название и описание
- Модули нумеруются последовательно: order = 1, 2, 3...
- Описание модуля объясняет что студент изучит в этом разделе

ТЕМЫ:
- 2-5 тем на модуль
- Темы внутри модуля тематически связаны
- Темы нумеруются последовательно внутри модуля: order = 1, 2, 3...
- От простого к сложному (EASY → MEDIUM → HARD → EXPERT)
- prerequisites указывают на slug предыдущих тем (из любого модуля)
- Первая тема первого модуля без prerequisites

ИКОНКИ:
- Подбирай подходящие эмодзи для модулей и тем
- Иконки должны отражать содержание
