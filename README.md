...
# MADI Schedule Parser (опционально)
USE_MADI_PARSER=false
MADI_CACHE_TTL=3600
...
Система парсинга расписания с официального сайта МАДИ для получения актуальной информации о занятиях, экзаменах и кафедрах преподавателей.

- `USE_MADI_PARSER` — включить/выключить парсер расписания MADI (по умолчанию `false`)
  - `true` — использовать реальный парсинг с сайта MADI
  - `false` — использовать статические данные
- `MADI_CACHE_TTL` — время жизни кэша в секундах (по умолчанию `3600` = 1 час)
  - Как долго хранить распарсенные данные перед обновлением
  - Рекомендуется: 3600-7200 секунд
- `MADI_REQUEST_TIMEOUT` — таймаут запроса в миллисекундах (по умолчанию `10000` = 10 секунд)
  - Максимальное время ожидания ответа от сайта MADI
  - Рекомендуется: 5000-15000 мс
- `MADI_FALLBACK_TO_STATIC` — использовать статические данные при ошибке парсера (по умолчанию `true`)
  - `true` — при ошибке парсинга возвращать статическое расписание
  - `false` — при ошибке возвращать сообщение об ошибке
- `MADI_BASE_URL` — базовый URL сайта MADI (по умолчанию `https://www.madi.ru/tplan`)

**Примеры использования:**

```env
# Включить парсер с настройками по умолчанию
USE_MADI_PARSER=true

# Кастомная конфигурация
USE_MADI_PARSER=true
MADI_CACHE_TTL=7200          # Кэш на 2 часа
MADI_REQUEST_TIMEOUT=15000   # Таймаут 15 секунд
MADI_FALLBACK_TO_STATIC=true # Fallback на статику
```

**Примечание:** Парсер работает без дополнительных API ключей. При отключенном парсере (`USE_MADI_PARSER=false`) система использует статические данные расписания.
