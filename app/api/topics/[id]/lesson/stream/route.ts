...
function getStreamingTheoryPrompt(topicName: string, courseTitle: string, context: string, domain: Domain): string {
  const domainHints: Record<Domain, string> = {
    ...
    GENERAL: 'Адаптируй под тему: конкретные примеры, сравнительные таблицы, практические применения. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать код на языках программирования — это домен не про программирование. Если речь об инструменте или платформе (например n8n), используй ТОЛЬКО текстовое описание конфигураций или скриншоты форматов, НО НЕ ПИШИ КОД.',
  }
...
- 2-3 интерактивных quiz-блока ВНУТРИ текста (после объяснения концепции, не в конце). СТРОГО в формате: \`\`\`interactive:quiz\n{"question": "...", "options": ["A", "B"], "correct": 0, "explanation": "..."}\n\`\`\`
- ОБЯЗАТЕЛЬНО: Если домен НЕ PROGRAMMING, АБСОЛЮТНО ЗАПРЕЩЕНО использовать блоки кода (\`\`\`javascript, \`\`\`python и т.д). Используй просто текст!
